...
void process_mandelbrot_avx (Window *window, int *code_error)
{
        ...
        for (int iy = 0; iy < HEIGHT; iy++)
        {
                for (int ix = 0; ix < WIDTH; ix += 4)
                {
                        ...
                        volatile __m256i belong_mandelbrot = _mm256_set1_epi64x(N_MAX - 1);

                        check_point_mandelbrot_avx(mandel_coord_x, mandel_coord_y, &belong_mandelbrot, code_error);
                        ERR_RET();

                        set_vector_color(&window->window_config, (long long int *) &belong_mandelbrot, ix, iy, code_error);
                }
        }
}
...
inline void check_point_mandelbrot_avx (const __m256d start_coord_x, const __m256d start_coord_y, volatile __m256i *belong_mandelbrot, int *code_error)
{
        ...
        for (int i = 1; i < N_MAX; i++)
        {
                ...
                *belong_mandelbrot = _mm256_add_epi64(*belong_mandelbrot, _mm256_castpd_si256 (cmp));
        ...
        *belong_mandelbrot = _mm256_mul_epi32(*belong_mandelbrot, reverse_sign);
}
...
inline void set_vector_color (WindowConfig *window_config, const long long int *belong_mandelbrot, const int ix, const int iy, int *code_error)
{
        ...
        for (int i = 0; i < 4; i++)
        {
                set_color_pixel(window_config, belong_mandelbrot[i], ix + i, iy, code_error);
        }
}
