...
|Метод расчета множества Мандельброта | Уровень оптимизации | Рендеринга кадра | Расчет множества Мандельброта  |
|-------------------------------------|---------------------|------------------|--------------------------------|
| Наивный метод                       |         -O2         | 480.8            | 478.9                          |
| Наивный метод                       |         -O3         | 252.9            | 251.1                          |
| Метод основанный на массивах        |         -O2         | 1725.3           | 1723.5                         |
| Метод основанный на массивах        |         -O3         | 331.5            | 329.9                          |
| SIMD инструкции                     |         -O2         | 270.9            | 269.2                          |
| SIMD инструкции                     |         -O3         | 78.1             | 76.4                           |

## Почему уровень оптимизации -O3 дает такое большое увеличение быстродействия кода

Чтобы ответить на этот вопрос, посмотрим на ассемблерный листинг нашего кода. Для этого воспользуемся сайтом [godbolt](https://godbolt.org/). Скомпилируем функции расчета множества Мандельброта при помощи avx инструкций с флагом `-O3` и без него. Нетрудно заметить, что версия с уровнем оптимизации `-O3` задействует большее количество регистров и раскрывается в меньшее количество инструкций.

![overflow](/picture_for_readme/godbolt.png)
![overflow](/picture_for_readme/godbolt_1.png)
...
Ниже приведена таблица с величинами, которые характеризуют увеличение быстродействия кода. Наивный метод рендеринга, с уровнем оптимизации -O2 будем считать за `1`, а все остальные величины будем рассчитывать относительно нее.
